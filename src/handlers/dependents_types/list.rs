...
pub async fn list(
    ...
    Query(filters): Query<DependentTypeFilter>,
) -> impl IntoResponse {
    ...
    let count_query = format!("SELECT COUNT(*) FROM dependents_types {}", where_clause);
    ...
    let query = format!(
        "SELECT * FROM dependents_types {} ORDER BY name LIMIT {} OFFSET {}",
    ...
    match result {
        ...
        Err(error) => {
            eprintln!("Failed to fetch dependents_types: {}", error);
