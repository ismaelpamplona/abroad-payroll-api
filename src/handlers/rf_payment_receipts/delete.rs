...
pub async fn delete(Extension(pool): Extension<PgPool>, Path(id): Path<Uuid>) -> impl IntoResponse {
    let delete_query = sqlx::query("DELETE FROM rf_payment_receipts WHERE id = $1")
        .bind(&id)
        .execute(&pool);
    ...
    match delete_query.await {
        Ok(delete_result) => {
            if delete_result.rows_affected() == 0 {
                let not_found_id = ErrorDetail {
                    ...
                    message: format!("Receipt with id {} does not exist.", id),
                };
        ...
        Err(error) => {
            eprintln!("Failed to delete receipt: {}", error);
