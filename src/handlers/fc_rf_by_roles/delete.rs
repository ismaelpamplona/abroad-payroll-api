...
pub async fn delete(Extension(pool): Extension<PgPool>, Path(id): Path<Uuid>) -> impl IntoResponse {
    let delete_query = sqlx::query("DELETE FROM fc_rf_by_roles WHERE id = $1")
        .bind(&id)
        .execute(&pool);
    ...
    match delete_query.await {
        Ok(delete_result) => {
            if delete_result.rows_affected() == 0 {
                let not_found_id = ErrorDetail {
                    ...
                    message: format!("fc_rf_by_roles with id {} does not exist.", id),
                };
        ...
        Err(error) => {
            eprintln!("Failed to delete fc_rf_by_roles: {}", error);
